...
import { View, Text, StyleSheet, ScrollView, TouchableOpacity, Alert, Switch, TextInput, Modal, ActivityIndicator, Animated } from 'react-native';
...
export const ProfileScreen: React.FC = () => {
  ...
  const handleAnalyticsToggle = async (value: boolean) => {
    try {
      ...
      if (!value) {
        Alert.alert(
          'Analytics Disabled',
          'Your usage data will not be collected',
          [{ text: 'OK' }]
        );
      }
    } catch (error) {
  ...
  const userStats = [
    ...
    { label: 'Goals Completed', value: `${stats.goalsCompleted}/${stats.totalGoals}`, icon: 'checkmark-circle' },
  ];
    ...
    try {
      ...
      if (emailResult.success) {
        ...
        Alert.alert(
          ...
          [
            { text: 'OK' },
            { text: 'View Data', onPress: () => console.log(exportedData) }
          ]
        );
      }
    } catch (error) {
        ...
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              // Delete account using Firebase service
              if (firebaseService.isInitialized()) {
                console.log('🔥 Using Firebase to delete account');
                const result = await firebaseService.deleteUserAccount();
                ...
                if (result.success) {
                  ...
                  // Show confirmation (will display after redirect to login)
                  setTimeout(() => {
                    Alert.alert(
                      'Account Deleted',
                  ...
                  setLoading(false);
                  Alert.alert('Delete Failed', result.error || 'Failed to delete account. Please try again.');
                }
              } else {
                ...
                setTimeout(() => {
                  Alert.alert(
                    'Account Deleted',
                    'Your local account data has been deleted.',
                    [{ text: 'OK' }]
                  );
                }, 500);
              }
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top', 'left', 'right']}>
      ...
      <View style={styles.userCard}>
        ...
        <View style={styles.userInfo}>
          ...
          <Text style={styles.userLevel}>
            {(user?.skillLevel || 'beginner').charAt(0).toUpperCase() + (user?.skillLevel || 'beginner').slice(1)} • {user?.position || 'Guard'}
          </Text>
        </View>
      ...
      <View style={styles.statsSection}>
        ...
        <View style={styles.statsGrid}>
          <View style={styles.statRow}>
            <GradientCard style={styles.statCard}>
              <View style={styles.statIconContainer}>
                <Ionicons name={userStats[0].icon as any} size={32} color={SHOOTRZ_THEME.colors.primary} />
              </View>
              <Text style={styles.statValue}>{userStats[0].value}</Text>
              <Text style={styles.statLabel} numberOfLines={2} adjustsFontSizeToFit minimumFontScale={0.8}>{userStats[0].label}</Text>
            </GradientCard>
            
            <GradientCard style={styles.statCard}>
              <View style={styles.statIconContainer}>
                <Ionicons name={userStats[1].icon as any} size={32} color={SHOOTRZ_THEME.colors.secondary} />
              </View>
              <Text style={styles.statValue}>{userStats[1].value}</Text>
              <Text style={styles.statLabel} numberOfLines={2} adjustsFontSizeToFit minimumFontScale={0.8}>{userStats[1].label}</Text>
            </GradientCard>
          </View>
          ...
          <View style={styles.statRow}>
            <GradientCard style={styles.statCard}>
              <View style={styles.statIconContainer}>
                <Ionicons name={userStats[2].icon as any} size={32} color={SHOOTRZ_THEME.colors.accent} />
              </View>
              <Text style={styles.statValue}>{userStats[2].value}</Text>
              <Text style={styles.statLabel} numberOfLines={2} adjustsFontSizeToFit minimumFontScale={0.8}>{userStats[2].label}</Text>
            </GradientCard>
            
            <GradientCard style={styles.statCard}>
              <View style={styles.statIconContainer}>
                <Ionicons name={userStats[3].icon as any} size={32} color={SHOOTRZ_THEME.colors.success} />
              </View>
              <Text style={styles.statValue}>{userStats[3].value}</Text>
              <Text style={styles.statLabel} numberOfLines={2} adjustsFontSizeToFit minimumFontScale={0.8}>{userStats[3].label}</Text>
            </GradientCard>
          </View>
        </View>
      </View>
      ...
      <View style={styles.settingsSection}>
        ...
        <GradientCard style={styles.settingItem}>
          ...
          <Switch
            ...
            trackColor={{ false: SHOOTRZ_THEME.colors.surfaceElevated, true: SHOOTRZ_THEME.colors.primary }}
            thumbColor={notifications ? SHOOTRZ_THEME.colors.textPrimary : SHOOTRZ_THEME.colors.textSecondary}
          />
        </GradientCard>
        ...
        <GradientCard style={styles.settingItem}>
          ...
          <Switch
            ...
            trackColor={{ false: SHOOTRZ_THEME.colors.surfaceElevated, true: SHOOTRZ_THEME.colors.primary }}
            thumbColor={darkMode ? SHOOTRZ_THEME.colors.textPrimary : SHOOTRZ_THEME.colors.textSecondary}
          />
        </GradientCard>
        ...
        <GradientCard style={styles.settingItem}>
          ...
          <Switch
            ...
            trackColor={{ false: SHOOTRZ_THEME.colors.surfaceElevated, true: SHOOTRZ_THEME.colors.primary }}
            thumbColor={analytics ? SHOOTRZ_THEME.colors.textPrimary : SHOOTRZ_THEME.colors.textSecondary}
          />
        </GradientCard>
      </View>
      ...
      <View style={styles.actionsSection}>
        ...
        <GradientCard style={styles.actionItem}>
          <TouchableOpacity style={styles.actionButton} onPress={() => Alert.alert('Help', 'Contact support at help@shootrz.com')}>
        ...
        <GradientCard style={styles.actionItem}>
          <TouchableOpacity style={styles.actionButton} onPress={() => Alert.alert('About', 'SHOOTRZ v1.0.0\nBuilt with React Native & AI')}>
            <Ionicons name="information-circle" size={24} color={SHOOTRZ_THEME.colors.textSecondary} />
