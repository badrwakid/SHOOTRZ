...
export const AnimatedScoreCard: React.FC<AnimatedScoreCardProps> = ({
  ...
  const safeScore = isNaN(score) ? 0 : Math.max(0, Math.min(score, 25)); // Clamp to 0-25
  const safeMaxScore = 25; // Always 25 for score cards

  // Only log in development and limit frequency
  if (__DEV__ && Math.random() < 0.01) { // Log ~1% of renders
  ...
  }, [safeScore, delay]); // Use safeScore instead of score/maxScore to prevent unnecessary re-renders
