...
function generateFeedback(components: any): string[] {
  ...
  const totalScore =
    components.elbow.score +
    components.balance.score +
    components.release.score +
    components.alignment.score;
  ...
  const lowestComponent = Object.entries(components).reduce(
    (lowest, [key, value]: [string, any]) => {
  ...
  const actionableTips = {
    elbow:
      '💪 Practice Tip: Do 50 form shots close to the basket, focusing solely on elbow position.',
    balance:
      '💪 Practice Tip: Practice shooting off one leg to improve your balance and stability.',
